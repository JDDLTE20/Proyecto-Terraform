...
resource "aws_route_table" "ROUTES-PRIVATE-A" {
      ...
      route {
          cidr_block = "0.0.0.0/0"
          nat_gateway_id = aws_nat_gateway.NAT-GW-PROYECTO.id
      }
...
resource "aws_lb" "LOADBALANCER-PROYECTO" {
  name = "LOADBALANCER-PROYECTO"
  internal = false
...
resource "aws_lb_listener" "LISTENER-WEB-PROYECTO" {
  load_balancer_arn = aws_lb.LOADBALANCER-PROYECTO.arn
  port = "80" 
  protocol = "HTTP"
...
resource "aws_lb_target_group" "TARGET-GROUP-PROYECTO" {
  name = "TARGET-GROUP-PROYECTO"
  port = 80
  protocol = "HTTP"
  vpc_id = aws_vpc.VPC-PROYECTO.id  
}
